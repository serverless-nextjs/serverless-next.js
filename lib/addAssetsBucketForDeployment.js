...
const getCFTemplatesWithBucket = async function(bucketName) {
  const compiledCfTemplate = this.serverless.service.provider.compiledCloudFormationTemplate ? addS3BucketToResources(
    bucketName,
    this.serverless.service.provider.compiledCloudFormationTemplate
  ): null;
  const coreCfTemplate = this.serverless.service.provider.coreCloudFormationTemplate ? addS3BucketToResources(
    bucketName,
    this.serverless.service.provider.coreCloudFormationTemplate
  ): null;

  return Promise.all([compiledCfTemplate, coreCfTemplate]);
};
