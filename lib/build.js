...
module.exports = async function() {
  ...
  nextPages.forEach(page => {
    ...
    // const now = Date.now() * Math.floor((Math.random() + count++));
    count++;
    // const name = `waw-${stage}-${now}`;
    const name = functionName.slice(0, 60)
    this.serverless.service.functions[`${name}${count}`] = page.serverlessFunction[functionName];
  });
