...
module.exports = (nextBuildDir, pluginBuildDirObj) => {
  logger.log("Copying next pages to plugin tmp build folder");

  const pagesBuildDir = path.join(nextBuildDir, "serverless/pages");

  return pluginBuildDirObj.setupBuildDir().then(() => {
    return readdirAsync(pagesBuildDir).then(pageFiles => {
