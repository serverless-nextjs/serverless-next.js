...
### Enviroment variables from other components

Enviroment variables can be made available to the next build command 
 by specifying the `buildEnv` input in `serverless.yml`:
...
myNextApp:
  ...
  inputs:
    buildEnv:
      MY_ENV_VAR: ${otherComponent.output}
```

You can then use the variables in `next.config.js` like so:
...
module.exports = {
  target: "serverless",
  env: {
    MY_NEXT_VAR: process.env.MY_ENV_VAR
  }
};
```
...
| buildEnv   | `Object` | `null`        | Enviroment variables to be made available during build. |
