// find a way to share type definition with serverless component v1
// @ts-ignore
...
class NextjsComponent extends Component {
  ...
  async deploy(
    ...
    const [defaultBuildManifest, apiBuildManifest] = await Promise.all([
      this.readDefaultBuildManifest(src),
      this.readApiBuildManifest(src)
    ]);

    const bucketUrl = defaultBuildManifest.bucketUrl;
    ...
    if (hasAPIPages) {
      const apiEdgeLambdaInput: LambdaInput = {
        ...
        code: join(src, API_LAMBDA_CODE_DIR),
        role: {
    ...
    const defaultEdgeLambdaInput: LambdaInput = {
      ...
      handler: "index.handler",
      code: join(src, DEFAULT_LAMBDA_CODE_DIR),
      role: {
        service: ["lambda.amazonaws.com", "edgelambda.amazonaws.com"],
        policy: {
          arn:
