...
type BuildOptions = {
  ...
  experimentalOutputFileTracing?: boolean;
};
...
const defaultBuildOptions = {
  ...
  useV2Handler: false,
  experimentalOutputFileTracing: false
};
...
class Builder {
  ...
  constructor(
    ...
    this.nextTargetDir = path.join(
      this.dotNextDir,
      buildOptions && buildOptions.experimentalOutputFileTracing
        ? "server"
        : "serverless"
    );
  ...
  async copyTraces(
    ...
    if (this.buildOptions.experimentalOutputFileTracing) {
      const allSsrPages = [
  ...
  async copyChunks(handlerDir: string): Promise<void> {
    const { useServerlessTraceTarget, experimentalOutputFileTracing } =
      this.buildOptions;
    if (useServerlessTraceTarget || experimentalOutputFileTracing) return;
  ...
  async copyJSFiles(handlerDir: string): Promise<void> {
    ...
    if (this.buildOptions.experimentalOutputFileTracing) return;
  ...
  async build(debugMode?: boolean): Promise<void> {
     ...
     */
    if (experimentalOutputFileTracing) {
