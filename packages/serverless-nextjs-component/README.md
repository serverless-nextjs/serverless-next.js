...
| region        | `string`          | `us-east-1`              | The region where s3 bucket resides (and the cloudfront distribution reports as the main region although they are deployed to every edge)                                                                                                                             |
...
You can configure the `region` for resources like `s3` and `cloudfront` via the region input configuration (but the lambda edge region isn't affected with this).
